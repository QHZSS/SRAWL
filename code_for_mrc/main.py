...
def main():
    ...
    if args.do_train:
        ...
        optimizer_parameters = [
            {'params': [p for n, p in model.named_parameters() if n not in no_decay and n !='eta'], 'weight_decay_rate': 0.01},
            {'params': [p for n, p in model.named_parameters() if n == 'eta'],'lr':0.1},
        ...
        for epoch in range(int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(tqdm(train_dataloader)):
                ...
                if global_step % args.eval_period == 0:
                    ...
                    logger.info("Step %d Train loss %.2f EM %.2f F1 %.2f R-L %.2f on epoch=%d" % (
                        global_step, np.mean(train_losses), f1[0]*100, f1[1]*100,f1[2]*100, epoch))
                    train_losses = []
                    if best_f1[1] < f1[1]:
