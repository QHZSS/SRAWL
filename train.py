...
def train(train_loader, train_table, model, model_bert,model_weight, opt, bert_config, tokenizer,
    ...
    for iB, t in enumerate(tqdm(train_loader)):
        ...
        g_sc, g_sa, g_wn, g_wc, g_wo, g_wv = get_g(sql_i,is_train=True)
        # get ground truth where-value index under CoreNLP tokenization scheme. It's done already on trainset.
        g_wvi_corenlp = get_g_wvi_corenlp(t,is_train=True)
        ...
        try:
            #
            g_wvi = get_g_wvi_bert_from_g_wvi_corenlp(t_to_tt_idx, g_wvi_corenlp,is_train=True)
        except:
        ...
        pr_wc_sorted = sort_pr_wc(pr_wc, [i[0] for i in g_wc])
        pr_sql_i = generate_sql_i(pr_sc, pr_sa, pr_wn, pr_wc_sorted, pr_wo, pr_wv_str, nlu)

        # Cacluate accuracy
        g_sc_=[i[0] for i in g_sc]
        g_sa_=[i[0] for i in g_sa]
        g_wn_=[i[0] for i in g_wn]
        g_wc_=[i[0] for i in g_wc]
        g_wo_=[i[0] for i in g_wo]
        g_wvi_=[i[0] for i in g_wvi]

        cnt_sc1_list, cnt_sa1_list, cnt_wn1_list, \
        cnt_wc1_list, cnt_wo1_list, \
        cnt_wvi1_list, cnt_wv1_list = get_cnt_sw_list(g_sc_, g_sa_, g_wn_, g_wc_, g_wo_, g_wvi_,
                                                      pr_sc, pr_sa, pr_wn, pr_wc, pr_wo, pr_wvi,
                                                      [i[0] for i in sql_i], pr_sql_i,
                                                      mode='train')
        ...
        cnt_x1_list, g_ans, pr_ans = get_cnt_x_list(engine, tb, g_sc_, g_sa_, [i[0] for i in sql_i], pr_sc, pr_sa, pr_sql_i)
