...
class BertForQuestionAnswering(nn.Module):
    ...
    def __init__(self, config, device, n_class, loss_type, variant_id=0, tau=None, eta=[1.0,1.0,1.0]):
    ...
    def forward(self, batch, global_step=-1):
        ...
        if len(batch) == 7:
            ...
            if self.loss_type=='top3':
                
                start_weigh_loss=0
                end_weigh_loss=0
                switch_weigh_loss=0
                for k in range(1,5):
                    if(torch.all(start_losses[k]==0).item() and torch.all(end_losses[k]==0).item() and torch.all(switch_losses[k]==0).item() ):
                        start_losses[k]=start_losses[0]
                        end_losses[k]=end_losses[0]
                        switch_losses[k]=switch_losses[0]
                for l in range(0,3):        
                    start_weigh_loss+=start_losses[l]*torch.exp(-self.eta[l])+self.eta[l]*0.5
                    end_weigh_loss+=end_losses[l]*torch.exp(-self.eta[l])+self.eta[l]*0.5
                    switch_weigh_loss+=switch_losses[l]*torch.exp(-self.eta[l])+self.eta[l]*0.5
                print(self.eta)
                total_loss = torch.sum(start_weigh_loss+end_weigh_loss+switch_weigh_loss)
            elif self.loss_type == "first-only":
