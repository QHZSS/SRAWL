...
if __name__ == '__main__':
    ...
    # Set path
    path_h = '/mnt/sda/qhz/sqlova' # change to your home folder
    ...
    path_pred = os.path.join(path_save_analysis, f'results_{mode}_top3.jsonl')
    ...
    with open(args.source_file) as fs, open(args.pred_file) as fp:
        ...
        for ls, lp in tqdm(zip(fs, fp), total=count_lines(args.source_file)):
            ...
            if(correct ==0):
                try:
                    print(f"question: {ep['nlu']}")
                    print(f"result: {ep['query']}")
                    print(f"gold :{eg['sql']}")
                    print(f"result: {pred},{gold}")
                    print(f"table-id: {eg['table_id']}\n")
                except Exception as e:
                    a=1
