    ...
    if n_paragraphs is None:
        ...
        for predictions in all_predictions.values():
            ...
            for i, prediction in enumerate(predictions):
                ...
                if(max([rougel_score(prediction, gt) for gt in groundtruth])!=1):
                    print(f'prediction: {prediction}')
                    print(f'groudtruth: {groundtruth}')
                    print(f'{max([rougel_score(prediction, gt) for gt in groundtruth])}\n')
        for n, f1s_, ems_, rougels_ in zip(n_paragraphs, f1s, ems,rougels):
            logger.info("n=%d\tF1 %.2f\tEM %.2f\tR-L %.2f"%(n, np.mean(f1s_)*100, np.mean(ems_)*100,np.mean(rougels_)*100))
