...
def get_fields_1(t1, tables, no_hs_t=False, no_sql_t=False):
    ...
    tid1 = t1['table_id']
    sql_i1 = t1['sql'] #top5 ranked sqls
    sql_q1 = t1['query']
    if no_sql_t:
        sql_t1 = None
    else:
...
def get_fields(t1s, tables, no_hs_t=False, no_sql_t=False):
    ...
    for t1 in t1s:
        ...
        nlu_t.append(nlu_t1)
        tid.append(tid1)
        sql_i.append(sql_i1) #top5 ranked sqls
        sql_q.append(sql_q1)
        sql_t.append(sql_t1)
...
def get_g(sql_i,is_train=False):
    ...
    if not is_train:
        ...
        g_wv = []
        for b, psql_i1 in enumerate(sql_i):
            g_sc.append( [psql_i11["sel"] for psql_i11 in psql_i1] )
            g_sa.append( [psql_i11["agg"] for psql_i11 in psql_i1] )
            ...
            for i in psql_i1:
                conds = i['conds']
                if not i["agg"] < 0:
                    g_wn[-1].append( len( conds ) )
                    g_wc[-1].append( get_wc1(conds) )
                    g_wo[-1].append( get_wo1(conds) )
                    g_wv[-1].append( get_wv1(conds) )
                else:
                    raise EnvironmentError
        return g_sc, g_sa, g_wn, g_wc, g_wo, g_wv

def get_g_wvi_corenlp(t,is_train=False):
    if not is_train:
        g_wvi_corenlp = []
        for t1 in t:
        ...
        g_wvi_corenlp = []
        for t1 in t:
            g_wvi_corenlp.append( t1['wvis_corenlp'] )
        return g_wvi_corenlp
...
def get_g_wvi_bert_from_g_wvi_corenlp(wh_to_wp_index, g_wvi_corenlp,is_train=False):
    """
    Generate SQuAD style start and end index of wv in nlu. Index is for of after WordPiece tokenization.
    ...
    if is_train:
        ...
        for b, g_wvi_corenlp1 in enumerate(g_wvi_corenlp):
            ...
            for i_wn, g_wvi_corenlp11 in enumerate(g_wvi_corenlp1):
                g_wvi1.append([])
                for g_wvi_corenlp111 in g_wvi_corenlp11:
                    st_idx, ed_idx = g_wvi_corenlp111

                    st_wp_idx = wh_to_wp_index1[st_idx]
                    ed_wp_idx = wh_to_wp_index1[ed_idx]

                    g_wvi11 = [st_wp_idx, ed_wp_idx]
                    g_wvi1[-1].append(g_wvi11)
    ...
    for b, g_wvi1 in enumerate(g_wvi):
        ...
        if g_wn1 != pr_wn1:
            ...
            for i_wn, idx11 in enumerate(idx1):
                ...
                if g_wvi11 != pr_wvi11:
                    flag = False
                    #print(g_wvi1, g_wvi11)
                    #print(pr_wvi1, pr_wvi11)
                    #input('')
                    break
            if flag:
                cnt_list.append(1)
